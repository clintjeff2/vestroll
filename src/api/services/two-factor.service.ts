...
const TOTP_CONFIG = {
  ...
  digits: 6 as const,
  period: 30,
  window: 1, // ±1 step (30 seconds tolerance)
  issuer: "VestRoll",
};
...
const SECURITY_CONFIG = {
  ...
  backupCodeCount: 10,
  trustedDeviceDuration: 30 * 24 * 60 * 60 * 1000, // 30 days in ms
  saltRounds: 12,
};
...
export class TwoFactorService {
  ...
  static async verifyTOTP(token: string, secret: string): Promise<boolean> {
    try {
      // epochTolerance is in seconds. window of 1 step = 30 seconds tolerance
  ...
  static async generateQRCode(email: string, secret: string): Promise<string> {
    ...
    // Build otpauth URI using the functional API
  ...
  static generateBackupCodes(): string[] {
    ...
    for (let i = 0; i < SECURITY_CONFIG.backupCodeCount; i++) {
      // Generate format: XXXX-XXXX-XXXX (alphanumeric uppercase)
  ...
  static async verifySetup(userId: string, totpCode: string): Promise<{
    ...
    // Use a transaction to enable 2FA and store backup codes
